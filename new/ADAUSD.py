...
import threading
import queue
api_key = '2usq6wWOavbc8U4ZlQ4OekBqsd6GmdAwr0TbsYM1fxxGWP5DyLk4xh7tmM0cIKah'
api_secret = 'eRn45Budb9RLnvxO91p1y1L7QW4njsiDTr2qpnWoSqb7uWmHoX7PRY7bwLMNZfAL'
...
while True:
    ...
    for symbol in list(open_trades.keys()):  # Create a copy of keys with list()
        if symbol == 'BTCBUSD':
            ...
            if pnl is not None:
                ...
                if pnl < -100:  # Trigger stop loss if PNL is less than -$100
    ...
    if previous_text is not None and text.strip() != previous_text:
        ...
        if text.strip() == "Alert:ShortSignalonBICUSD" or text.strip() == "Alert:StrongShortSignalonBICUSD":
            print("Take profit 1 reached for short")
            # Close 75% of the trade
            sell('BTCBUSD', 1, "short")
        elif text.strip() == "Alert:LongSignalonBICUSD" or text.strip() == "Alert:StrongLongSignalonBICUSD":
            print("Take profit 1 reached for short")
            # Close 75% of the trade
            buy('BTCBUSD', 1, "long")
        ...
        elif text.strip() == "Alert:ShortTakeProfit1onBICUSD":
             print("Take profit 1 reached for short")
                    # Close 75% of the trade
        ...
        elif text.strip() == "Alert:LongTakeProfit1onBICUSD":
        ...
        elif text.strip() == "Alert:ShortTakeProfit2onBICUSD":
            print("Take profit 2 reached for short")
            # Close 100% of the trade
            take_profit('BTCBUSD', 1, "short")

        elif text.strip() == "Alert:LongTakeProfit2onBICUSD":
            print("Take profit 2 reached for long")
            # Close 100% of the trade
            take_profit('BTCBUSD', 1, "long")

        elif text.strip() == "Alert:LongStopLossonBICUSD" or text.strip() == "Alert:LongExitonBICUSD":
            print("long stop loss")
            # Close 100% of the trade
            take_profit('BTCBUSD', 1, "long")

        elif text.strip() == "Alert:ShortStopLossonBICUSD" or text.strip() == "Alert:ShortExitonBICUSD":
