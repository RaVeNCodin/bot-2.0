...
while True:
    ...
    for symbol in list(open_trades.keys()):  # Create a copy of keys with list()
        ...
        if symbol == 'ETHBUSD':
            ...
            if pnl is not None:
                ...
                if pnl < -110:  # Trigger stop loss if PNL is less than -$100
    ...
    if previous_text is not None and text.strip() != previous_text:
        ...
        if text.strip() == "0000010000" or text.strip() == "Alert:StrongShortSignalonETHUSD":
            print("Take profit 1 reached for short")
            # Close 75% of the trade
            #تغيير اسم العملة 2
            sell('ETHBUSD', 5, "short")
        elif text.strip() == "0000020000" or text.strip() == "Alert:StrongLongSignalonETHUSD":
            print("Take profit 1 reached for short")
            # Close 75% of the trade
            #تغيير اسم العملة 3
            buy('ETHBUSD', 5, "long")
