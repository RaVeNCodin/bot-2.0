...
while True:
    ...
    for symbol in list(open_trades.keys()):  # Create a copy of keys with list()
        if symbol == 'ETHBUSD':
            ...
            if pnl is not None:
                ...
                if pnl < -250:  # Trigger stop loss if PNL is less than -$100
    ...
    if previous_text is not None and text.strip() != previous_text:
        ...
        if text.strip() == "Alert:ShortSignalonETHUSD" or text.strip() == "Alert:StrongShortSignalonETHUSD":
            print("Take profit 1 reached for short")
            # Close 75% of the trade
            sell('ETHBUSD', 10, "short")
        elif text.strip() == "Alert:LongSignalonETHUSD" or text.strip() == "Alert:StrongLongSignalonETHUSD":
            print("Take profit 1 reached for short")
            # Close 75% of the trade
            buy('ETHBUSD', 10, "long")
        ...
        elif text.strip() == "Alert:ShortTakeProfit1onETHUSD":
            print("Take profit 1 reached for short")
            # Close 75% of the trade
            take_profit('ETHBUSD', 0.75, "short")
        ...
        elif text.strip() == "Alert:LongTakeProfit1onETHUSD":
            print("Take profit 1 reached for long")
            # Close 75% of the trade
            take_profit('ETHBUSD', 0.75, "long")
            initial_pnl = calculate_pnl(symbol)
            print(f"int PNL for {symbol}: {initial_pnl}")
            # Start the profit stop loss function in a separate thread and pass the queue as an argument
            threading.Thread(target=profit_stop_loss_long, args=(symbol, initial_pnl, profit_stop_loss_queue)).start()

        elif text.strip() == "Alert:ShortTakeProfit2onETHUSD":
            print("Take profit 2 reached for short")
            # Close 100% of the trade
            take_profit('ETHBUSD', 1, "short")

        elif text.strip() == "Alert:LongTakeProfit2onETHUSD":
            print("Take profit 2 reached for long")
            # Close 100% of the trade
            take_profit('ETHBUSD', 1, "long")

        elif text.strip() == "Alert:LongStopLossonETHUSD" or text.strip() == "Alert:LongExitonETHUSD":
            print("long stop loss")
            # Close 100% of the trade
            take_profit('ETHBUSD', 1, "long")

        elif text.strip() == "Alert:ShortStopLossonETHUSD" or text.strip() == "Alert:ShortExitonETHUSD":
    ...
    time.sleep(1)  # Adjust the sleep time as needed    time.sleep(1)  # Adjust the sleep time as needed