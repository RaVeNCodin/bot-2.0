...
while True:
    ...
    if previous_text is not None and text.strip() != previous_text:
        ...
        if text.strip() == "0000010000" or text.strip() == "Alert:StrongShortSignalonLTCUSD":
            print("Take profit 1 reached for short")
            # Close 75% of the trade
            #تغيير اسم العملة 2
            sell('LTCBUSD', 100, "short")
        elif text.strip() == "0000020000" or text.strip() == "Alert:StrongLongSignalonLTCUSD":
            print("Take profit 1 reached for short")
            # Close 75% of the trade
            #تغيير اسم العملة 3
            buy('LTCBUSD', 100, "long")
             ...
             print("Take profit 1 reached for short")
                    # Close 75% of the trade
             take_profit('LTCBUSD', 0.75, "short")
            ...
            # Close 75% of the trade
            take_profit('LTCBUSD', 0.75, "long")
            initial_pnl = calculate_pnl(symbol)
            print(f"int PNL for {symbol}: {initial_pnl}")
            # Start the profit stop loss function in a separate thread and pass the queue as an argument
            threading.Thread(target=profit_stop_loss_long, args=(symbol, initial_pnl, profit_stop_loss_queue)).start()

        elif text.strip() == "Alert:ShortTakeProfit2onLTCUSD":
            print("Take profit 2 reached for short")
            # Close 100% of the trade
            #تغيير اسم العملة 4
            take_profit('LTCBUSD', 1, "short")

        elif text.strip() == "Alert:LongTakeProfit2onLTCUSD":
            print("Take profit 2 reached for long")
            # Close 100% of the trade
            #تغيير اسم العملة 5
            take_profit('LTCBUSD', 1, "long")

        elif text.strip() == "Alert:LongStopLossonLTCUSD" or text.strip() == "Alert:LongExitonLTCUSD":
            print("long stop loss")
            # Close 100% of the trade
            #تغيير اسم العملة 6
            take_profit('LTCBUSD', 1, "long")

        elif text.strip() == "Alert:ShortStopLossonLTCUSD" or text.strip() == "Alert:ShortExitonLTCUSD":
